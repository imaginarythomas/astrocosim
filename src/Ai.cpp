            ...
            for (Actor **iterator = engine.actors.begin(); iterator != engine.actors.end(); iterator++){
                ...
                if (actor->pickable && actor->x == owner->x && actor->y == owner->y){
                    if (actor->pickable->pick(actor, owner)){
                        found = true;
                        engine.gui->message(TCODColor::lightGrey, "You pick up a %s", actor->name);
                        break;
                    }else if(!found){
                        found = true;
                ...
                if (!found){
                    engine.gui->message(TCODColor::red, "There's nothing here");
                }
                engine.gameStatus = Engine::NEW_TURN;
            }
        }
...
bool PlayerAi::moveOrAttack(Actor *owner, int targetx, int targety){
    ...
    for (Actor **iterator = engine.actors.begin(); iterator != engine.actors.end(); iterator++){
        ...
        if (corpseOrItem &&  actor->x == targetx && actor->y == targety){
            engine.gui->message(TCODColor::lightGrey, "There is a %s here", actor->name);
        }
    }
    owner->x = targetx;
    owner->y = targety;
...
void MachineAi::update(Actor* owner){
    //TODO
}
...
void IdleNPCAi::update(Actor *owner){
    TCODRandom *rng = TCODRandom::getInstance();
    int dx = rng->getInt(-1, 1);
    int dy = rng->getInt(-1, 1);
